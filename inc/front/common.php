...
function secupress_rename_admin_username_logout() {
	...
	if ( ! empty( $_POST ) || defined( 'DOING_AJAX' ) || defined( 'DOING_AUTOSAVE' ) || defined( 'DOING_CRON' ) || 'admin-post.php' === $pagenow || ! is_user_logged_in() ) {
		return;
	}
	...
	if ( (int) $data['ID'] !== (int) $current_user->ID || 'admin' !== $current_user->user_login ) {
		return;
	}
