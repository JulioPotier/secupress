...
class SecuPress_Scan_Bad_Old_Plugins extends SecuPress_Scan implements SecuPress_Scan_Interface {
	...
	protected function init() {
		...
		$this->more  = __( 'Avoid using a plugin that has been removed from the official repository, and avoid using a plugin that have not been maintained for two years at least.', 'secupress' );

		if ( is_network_admin() ) {
			$this->more_fix  = __( 'Select removed and old plugins plugins to be deleted.', 'secupress' );
			$this->more_fix .= '<br/>' . __( 'Will be fixable soon.', 'secupress' ); // RC2 ////.
			$this->fixable   = false;
		} elseif ( ! is_multisite() ) {
			$this->more_fix = __( 'Select and delete removed and  old plugins plugins.', 'secupress' );
		} else {
