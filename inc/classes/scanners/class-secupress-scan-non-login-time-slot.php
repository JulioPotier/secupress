...
class SecuPress_Scan_Non_Login_Time_Slot extends SecuPress_Scan implements SecuPress_Scan_Interface {
	...
	protected function init() {
		...
		if ( $timings && is_array( $timings ) ) {
			...
			$this->more_fix .= '<br/>' . sprintf( __( 'It will prevent anyone to log in from %1$s to %2$s (this time slot can be changed in the module settings).', 'secupress' ), "<strong>$from</strong>", "<strong>$to</strong>" );
