...
class SecuPress_Scan_Common_Flaws extends SecuPress_Scan implements SecuPress_Scan_Interface {
	...
	protected function init() {
		$this->title = __( 'Check if your website is vulnerable to the most common flaws.', 'secupress' );
	...
	public function scan() {
		 ...
		 */
		++$nbr_tests;
		$response = wp_remote_get( add_query_arg( secupress_generate_key( 6 ), 'wp-config.php', user_trailingslashit( home_url() ) ), array( 'redirection' => 0, 'timeout' => $this->get_timeout(), 'headers' => array( 'X-SecuPress-Origin' => __CLASS__ ) ) );
