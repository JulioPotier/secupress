...
function secupress_warning_wp_config_permissions() {
	...
	<div class="error">
		...
		<p>
			<?php
			printf( __( '%s: It seems we don\'t have <a href="%s" target="_blank">writing permissions</a> on <code>wp-config.php</code> file.', 'secupress' ), '<strong>' . SECUPRESS_PLUGIN_NAME . '</strong>', 'http://codex.wordpress.org/Changing_File_Permissions' );
			echo '<br>';
...
function secupress_warning_htaccess_permissions() {
	...
	<div class="error">
		<a href="<?php echo wp_nonce_url( admin_url( 'admin-post.php?action=secupress_ignore&box='.__FUNCTION__ ), 'secupress_ignore_'.__FUNCTION__ ); ?>" class="secupress-cross"><div class="dashicons dashicons-no"></div></a>
		<p><strong><?php echo SECUPRESS_PLUGIN_NAME; ?></strong>: <?php printf( __( 'If you had <a href="%1$s" target="_blank">writing permissions</a> on <code>.htaccess</code> file, %2$s could do this automatically. This is not the case, so here are the rewrite rules you have to put in your <code>.htaccess</code> file for %2$s to work correctly. Click on the field and press Ctrl-A to select all.', 'rocket' ), 'http://codex.wordpress.org/Changing_File_Permissions', '<strong>' . SECUPRESS_PLUGIN_NAME . '</strong>' ) . '<br>' . __( '<strong>Warning:</strong> This message will popup again and its content may be updated when saving the options', 'secupress' ); ?></p>
