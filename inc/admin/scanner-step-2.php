...
echo '<div>' . __( 'Checked items will be automatically fixed', 'secupress' ) . '</div>';
?>
<button type="submit">Fix all checked issues</button>
<?php
foreach ( $secupress_tests as $module_name => $class_name_parts ) {
	...
	<div class="secupress-table-prio-all<?php echo ( $is_subsite ? '' : ' secupress-table-prio-' . $module_name ); ?>">
		...
		<div class="secupress-prio-title prio-<?php echo $module_name; ?>">
			...
			<span class="alignright"><input type="checkbox"></span>
			<span class="alignright"><a href="<?php echo secupress_admin_url( 'modules', $module_name ); ?>"><?php esc_html_e( 'Toggle group check', 'secupress' ); ?></a></span>
		</div>
		...
		if ( ! $is_subsite ) {
			...
			$class_name_parts        = $this_prio_bad_scans; //array_merge( $this_prio_good_scans, $this_prio_warning_scans, $this_prio_bad_scans );
