...
class SecuPress_Action_Logs extends SecuPress_Logs {
	...
	protected function log( $type, $target, $data = null ) {
		...
		if ( false === strpos( wp_json_encode( isset( $log['data'] ) && isset( $log['data']['args'] ) && isset( $log['data']['args']['headers'] ) ? $log['data']['args']['headers'] : '' ), 'Decalog-No-Log' ) ) {
			\DecaLog\Engine::eventsLogger( SECUPRESS_PLUGIN_SLUG )->log( $this->psr_levels[ isset( $log['critic'] ) ? $log['critic'] : 'unknown' ] ? $this->psr_levels[ isset( $log['critic'] ) ? $log['critic'] : 'unknown' ] : $this->psr_levels['unknown'], $log_inst->get_message() );
		}
