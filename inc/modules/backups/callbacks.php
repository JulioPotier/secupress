...
function __secupress_download_backup_ajax_post_cb() {
	...
	$file = glob( secupress_get_backup_path() . '*' . $_GET['file'] . '*.{zip,sql}', GLOB_BRACE );
	if ( $file ) {
		$file = reset( $file );
	} else {
		wp_nonce_ays( '' );
...
function __secupress_delete_backup_ajax_post_cb() {
	...
	$files = glob( secupress_get_backup_path() . '*' . $_GET['file'] . '*.{zip,sql}', GLOB_BRACE );
	if ( ! $files ) {
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			wp_send_json_error();
		} else {
...
function __secupress_delete_backups_ajax_post_cb() {
	...
	$files = glob( secupress_get_backup_path() . '*.{zip,sql}', GLOB_BRACE );
	if ( ! $files ) {
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			wp_send_json_error();
		} else {
