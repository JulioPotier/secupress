	...
	if ( user_can( $user, 'exist' ) ) {
		...
		if ( $time >= time() ) {
			...
			$redirect_to = add_query_arg( array(
				'action'     => 'doubleauth_lost_form_' . sanitize_key( $_POST['doubleauth_lost_method'] ),
				'token'      => $CLEAN['token'],
...
function __secupress_doubleauth_lost_form_backupcode() {
	...
	if ( ! $CLEAN['token'] || ! $user || ! secupress_doubleauth_get_user_option( 'lost', $CLEAN['uid'] ) ) {
		...
		if ( $time >= time() ) {
			...
			if ( $timeslot = in_array( $_POST['otp'], $backupcodes ) ) {
				...
				$k = array_search( $_POST['otp'], $backupcodes );
				$backupcodes[ $k ] = false;
				 ...
				 *
				 */
				$secure_cookie = apply_filters( 'secure_signon_cookie', is_ssl(), $args ); // we don't have the real password, just pass something.
	...
	if ( $show_form ) {
		...
		<form name="loginform" id="loginform" action="<?php echo esc_url( site_url( 'wp-login.php?action=doubleauth_lost_form_backupcode' ) ); ?>" method="post">
			...
			<p class="submit">
				<input type="submit" name="wp-submit" id="main-submit" class="button button-primary button-large button-full" value="<?php esc_attr_e( 'Verify', 'secupress' ); ?>" />
			</p>
		</form>
		<?php
		login_footer( 'otp' );
...
function __secupress_doubleauth_lost_form_backupmail() {
	...
	if ( user_can( $user, 'exist' ) && is_email( get_user_option( 'backup_email', $user->ID ) ) ) {
		...
		$subject = sprintf( __( '[%1$s] Mobile Authenticator Backup Code request', 'secupress' ), get_bloginfo( 'name' ) );
		/**
		 *
		 */
		$subject = apply_filters( 'secupress.doubleauth.backupcode_email.subject', $subject );

		$message = sprintf( __( 'Hello %1$s, you asked for a backup code, here it comes: %2$s.' ), $user->display_name, $code );
		/**
		 *
		 */
		$message = apply_filters( 'secupress.doubleauth.backupcode_email.message', $message );

		wp_mail( get_user_option( 'backup_email', $user->ID ), $subject, $message, 'content-type: text/html' );

		$redirect_to = add_query_arg( array(
			'action'     => 'doubleauth_lost_form_backupcode',
			'token'      => $CLEAN['token'],
			'rememberme' => $CLEAN['rememberme'],
			'emailed'    => 1,
		), wp_login_url() );
			...
			<p>
				<label>
					...
					<span class="authinfo">
						...
						<em><?php _e( 'Enter the verification code generated by your mobile application.', 'secupress' ); ?></em>
					</span>
					<input type="text" placeholder="******" class="doubleauth" onkeypress="return event.charCode >= 48 && event.charCode <= 57 || event.charCode == 13" name="otp" id="otp" maxlength="6" size="20" style="ime-mode: inactive;" />
				</label>
			</p>
			...
			<div id="help_info" class="hide-if-js">
				...
				<div class="blocks">
					<button type="submit" name="doubleauth_lost_method" value="backupcode">
						<span class="dashicons dashicons-format-aside" aria-hidden="true"></span> <?php _e( 'Use a backup code', 'secupress' ); ?>
					</button>
					<button type="submit" name="doubleauth_lost_method" value="backupmail">
						<span class="dashicons dashicons-email" aria-hidden="true"></span>  <?php _e( 'Send me a new backup code on my recovery email', 'secupress' ); ?>
					</button>
					<button type="submit" name="doubleauth_lost_method" value="askadmin">
						<span class="dashicons dashicons-sos" aria-hidden="true"></span>  <?php _e( 'Ask "Han Solo"\'s admin for help getting back into your account', 'secupress' ); ?>
					</button>
				</div>
			</div>
		</form>
	...
	jQuery( document ).ready( function($) {
		...
		$( "#doubleauth_renew_app_password" ).on( "click", function( e ) {
			...
			if ( confirm( '<?php echo esc_js( __( "Renewing your application password will forbid old password to work again.\nAre you sure to continue?", 'secupress' ) ); ?>' ) ) {
				...
				$( "#app_password" ).html( '<img src="<?php echo admin_url( '/images/wpspin_light.gif' ); ?>" alt="" role="presentation" />' );
				...
				$.get( href.replace( "admin-post", "admin-ajax" ), function( data ) {
					...
					if ( data.success ) {
						orig = data.data;
						code = orig.substr( 0, 3 ) + '<span style="letter-spacing:1em">' + orig[3] + '</span>' + orig.substr( 4, 3 ) + '<span style="letter-spacing:1em">' + orig[7] + '</span>' + orig.substr( 8, 3 ) + '<span style="letter-spacing:1em">' + orig[11] + '</span>' + orig.substr( 12, 4 );
				...
				$.get( href.replace( "admin-post", "admin-ajax" ), function( data ) {
					...
					if ( data.success ) {
						...
						for ( index in data.data ) {
							orig = data.data[ index ];
							code = orig.substr( 0, 3 ) + '<span style="letter-spacing:1em">' + orig[3] + '</span>' + orig.substr( 4, 8 );
...
function __secupress_doubleauth_regen_secret_ajax_post_cb() {
	...
	if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'secupress_doubleauth_regen_secret' ) ) {
		...
		array_map( 'secupress_doubleauth_delete_user_option', array(
			'verified', 'skip', 'secret', 'app_password', 'timeout', 'lasttimeslot', 'lost', 'token', 'backupcodes',
		) );
...
function secupress_doubleauth_redirect() {
	...
	$args          = array();
	$args['head']  = '<script src="' . esc_url( site_url( '/wp-includes/js/jquery/jquery.js' ) ) . '" type="text/javascript"></script>' . "\n";
	...
	<form class="wrap" action="<?php echo secupress_get_current_url( 'raw' ); ?>" method="post">
		...
		switch ( $step ) {
			...
			case 1:
				...
				$args['head'] .= '<script src="' . esc_url( plugins_url( 'inc/js/jquery.qrcode-0.12.0' . $suffix . '.js', __FILE__ ) ) . '" type="text/javascript"></script>' . "\n";
				$args['head'] .= '<script type="text/javascript">jQuery(document).ready( function( $ ) { ' . "\n" .
					...
					'$( ".hide-if-no-js" ).show();' . "\n" .
					'$( "#doubleauth_qrcode" ).text( "" ).qrcode( { "render":"image", "background":"#ffffff", "size":200, "text":qrcode } );' . "\n" .
				...
				<div class="hide-if-js">
					...
					<span class="description">
						<img src="https://cdn2.iconfinder.com/data/icons/touch-gestures-10/24/Mobile-Touch-128.png" todo="////" alt="" role="presentation">
