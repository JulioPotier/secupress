	...
	secupress_add_settings_field(
		...
		array(
			...
			array(
				...
				'description'  => __( 'When you log in, you\'ll receive an email with a link to be clicked, then, you\'ll be logged in.' ),
			),
			array(
				'type'	=> 'helper_warning',
				'name'	=> 'plugin_' . $pluginnow,
				'class'	=> array( 'hidden', 'block-hidden', 'block-emaillink', 'block-' . 'plugin_' . $pluginnow ),
				'description'  => sprintf( __( 'Is <code>%1$s</code> a valid email address? If not, <a href="%2$s">change it before logging out</a>.' ), $current_user->user_email, get_edit_profile_url( $current_user->ID ) . '#email' )
			),			
		)
	);
