...
(function($, d, w, undefined) {
	...
	// Delete all backups
	$( "#submit-delete-db-backups" ).on( "click", function( e ) {
		e.preventDefault();

		swal( {
				title:               l10nmodules.areYouSure,
				text:                l10nmodules.confirmDeleteBackups,
				html:                true,
				type:                'warning',
				...
				confirmButtonText:   l10nmodules.yesDeleteAll,
			},
			function() {

				$.get( $( "#form-delete-db-backups" ).attr( "action" ).replace( "admin-post.php", "admin-ajax.php" ) )
				.done( function( r ) {
					swal.close();
					if ( undefined != r.success && r.success ) {
						$( "#form-delete-db-backups fieldset p, #form-delete-db-backups fieldset hr" ).remove();
						secupress_update_backup_visibility();
					} else {
						swal( { title: l10nmodules.error, type: 'error', text: l10nmodules.deleteAllImpossible, allowOutsideClick: true } );
					}
					secupress_update_available_backup_counter();
				} )
				.fail( function() {
					// Error
					swal( { title: l10nmodules.error, type: 'error', text: l10nmodules.unknownError, allowOutsideClick: true } );
				} );
			}
		);
	...
	// Delete a Backup
	$( "body" ).on( "click", ".a-delete-backup", function( e ) {
		e.preventDefault();

		var $t = this;
		$.get( $( $t ).attr( 'href' ).replace( "admin-post.php", "admin-ajax.php" ) )
		.done( function( r ) {
			if ( undefined != r.success && r.success ) {
				$( "#file-uniqid-" + $( $t ).attr( "data-file-uniqid" ) ).css( "backgroundColor", "#D73838" ).hide( "slow", 
					function() { 
						$( this ).remove(); 
						secupress_update_available_backup_counter(); 
						secupress_update_backup_visibility();
				...
				swal( { title: l10nmodules.error, type: 'error', text: l10nmodules.deleteOneImpossible, allowOutsideClick: true } );
			}
	...
	$( "#submit-backup-db" ).on( "click", function( e ) {
		...
		$( this ).val( $( this ).attr( "data-loading-i18n" ) );
		$( "#secupress-db-backup-spinner" ).show();

		$.post( $( "#form-do-db-backup" ).attr( "action" ).replace(  "admin-post.php", "admin-ajax.php" ) )
		.done( function( r ) {
			$( $t ).removeAttr( "disabled" );
			$( $t ).val( $( $t ).attr( "data-original-i18n" ) );
			$( "#secupress-db-backup-spinner" ).hide();
			if ( undefined != r.success && r.success ) {
				$( "#form-delete-db-backups fieldset" ).prepend( r.data.replace( "db-backup-row", "db-backup-row hidden" ) );
				$( ".db-backup-row.hidden:not(:visible)" ).css( "backgroundColor", "#88BA0E" ).show( "slow", function(){ $( this ).css( "backgroundColor", "" ) } );
			} else {
				swal( { title: l10nmodules.error, type: 'error', text: l10nmodules.backupImpossible, allowOutsideClick: true } );
			}
			secupress_update_available_backup_counter();
			secupress_update_backup_visibility();
		} )
	...
	function secupress_update_available_backup_counter() {
		$( "#secupress-available-backups" ).text( $( ".db-backup-row" ).length );
	}

	function secupress_update_backup_visibility() {
		if ( $( ".db-backup-row" ).length == 0 ) {
